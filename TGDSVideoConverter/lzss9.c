...
void LZS_Decode(char *filename) {
  ...
  pak_buffer =(unsigned char*)Load(filename, (int*)&pak_len, LZS_MINIM, LZS_MAXIM);
  ...
  Save(filename, (char*)raw_buffer, raw_len);
...
void LZS_Encode(char *filename, int mode) {
  ...
  raw_buffer = (unsigned char*)Load(filename, (int*)&raw_len, RAW_MINIM, RAW_MAXIM);
  ...
  if (!(mode & LZS_FAST)) {
    ...
    new_buffer = (unsigned char*)LZS_Code(raw_buffer, raw_len, (int*)&new_len, mode);
  } else {
    new_buffer = (unsigned char*)LZS_Fast(raw_buffer, raw_len, (int*)&new_len);
  }
  if (new_len < pak_len) {
  ...
  Save(filename, (char*)pak_buffer, pak_len);
...
/*----------------------------------------------------------------------------*/
//generates LZSS best compressed objects to-be used with SWI decompress LZSS from WRAM
void LZS_EncodeFromBuffer(unsigned char *raw_buffer, unsigned int raw_len, unsigned int *pak_buffer, unsigned int *pak_len) {
...
void LZS_DecodeFromBuffer(unsigned char *pak_buffer, unsigned int pak_len, unsigned char * raw_buffer, unsigned int * raw_len) {
  unsigned char *pak, *raw, *pak_end, *raw_end;
  unsigned int   header, len, pos;
  unsigned char  flags, mask;

  printf("- decoding frame from buffer");
